...
Only runs when real calendar data (attendees + duration) is available.
Returns None for transcript-only sessions where these would be fabricated.
"""
import vertexai
...
def run(meeting_id: str, meeting_data: dict, notes_data: dict) -> dict | None:
    """
    ...
    Returns None if meeting_data does not contain real attendee/duration data,
    to avoid showing fabricated cost/time figures.
    """
    attendees     = meeting_data.get('attendees', [])
    duration_mins = meeting_data.get('duration_minutes')

    # Require both real attendee list and real duration.
    # A missing or zero duration means we have no calendar data to work from.
    if not attendees or not duration_mins:
        append_log(meeting_id, 'roi_agent',
                   'Skipping ROI — no real calendar data (attendees or duration missing).')
        return None
...
No markdown, no extra text."""
    ...
    append_log(meeting_id, 'roi_agent',
               f'ROI score: {roi_result["score"]}/100 ({roi_result["verdict"]}). '
               f'Cost: INR {cost_inr:,}')
    return roi_result